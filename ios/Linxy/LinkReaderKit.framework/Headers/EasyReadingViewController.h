...
/**
 Called when a mark is detected and a payoff is found. The state of the detector is set to stopped by the time this method is called.
 
  @param payoff The payoff that was found.
 
 @return `YES` if you want to present the payoff, 'NO' otherwise
...
/**
 ...
 # Basic Usage - Presenting Modally
     ...
     EasyReadingViewController *vc = [[EasyReadingViewController alloc] initWithClientID:kTestClientID secret:nil delegate:self success:^{
 ...
 In some cases, it may be desirable to insert the EasyReadingViewController's view directly into the view hierarchy of another view controller. In this case, create an instance of the EasyReadingViewController (ERCV) as you would normally. Next, add ERVC as a child view controller:
    ...
    -(void)viewDidLoad {
        [super viewDidLoad];
         self.easyReading = [[EasyReadingViewController alloc] initWithClientID:kTestClientID secret:nil delegate:self success:^{
            NSLog(@"Authorization complete!");
         } failure:^(NSError *error) {
            // Handle error
        }];
